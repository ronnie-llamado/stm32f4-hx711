...
inline void timer_set_prescaler(struct timer_t * self, uint32_t prescaler)
{
    LL_TIM_SetPrescaler(self->timx, prescaler);
}

inline void timer_set_period(struct timer_t * self, uint32_t period)
{
    LL_TIM_SetAutoReload(self->timx, period);
}

inline void timer_set_counter(struct timer_t * self, uint32_t count)
{
    LL_TIM_SetCounter(self->timx, count);
}

inline void timer_enable(struct timer_t * self)
{
    LL_TIM_EnableCounter(self->timx);
}

inline void timer_disable(struct timer_t * self)
{
    LL_TIM_DisableCounter(self->timx);
}

inline bool timer_is_update_set(struct timer_t * self)
{
    return (LL_TIM_IsActiveFlag_UPDATE(self->timx) == 1UL);
}

inline void timer_clear_update(struct timer_t * self)
{
